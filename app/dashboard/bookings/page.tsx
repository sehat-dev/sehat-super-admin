...
export default function BookingsPage() {
  ...
  const cancelBooking = async (bookingId: string, reason?: string) => {
    try {
      const reasonPrompt = reason || prompt('Enter cancellation reason (optional):');
  ...
  const getStatusBadge = (status: string) => {
    ...
    return { variant: variants[status] || 'outline', color: colors[status] || '' };
  };
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">Manage and view all bookings in the system</p>
          </div>
          <Button onClick={() => { fetchBookings(); fetchStats(); }} variant="outline">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b">
                        ...
                        <th className="text-left py-3 px-4 font-medium text-gray-700">Slot Date/Time</th>
                      ...
                      {bookings.map((booking) => {
                        ...
                        return (
                          <tr key={booking._id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="py-3 px-4">
                              <div className="flex flex-col">
                                <span className="font-medium text-sm">
                                  {booking.userId?.name || booking.userId?.sehatId || 'N/A'}
                                </span>
                                <span className="text-xs text-gray-500">{booking.userId?.sehatId}</span>
                              </div>
                            </td>
                            <td className="py-3 px-4">
                              <div className="flex flex-col">
                                <span className="font-medium text-sm">
                                  {booking.serviceDetails?.packageName || booking.serviceDetails?.serviceName || booking.serviceType}
                                </span>
                            ...
                            <td className="py-3 px-4">
                              <div className="flex flex-col">
                                <span className="font-medium text-sm">{booking.patientDetails.name}</span>
                                <span className="text-xs text-gray-500">
                                  {booking.patientDetails.age} yrs, {booking.patientDetails.gender}
                                </span>
                              </div>
                            </td>
                            <td className="py-3 px-4">
                              <div className="flex items-center space-x-2">
                                ...
                                <div className="flex flex-col">
                                  <span className="text-sm">{formatDateOnly(booking.slotDateTime)}</span>
                                  <span className="text-xs text-gray-500">
                            ...
                            <td className="py-3 px-4">
                              <div className="flex items-center space-x-2">
                                ...
                                <div className="flex flex-col">
                                  <span className="text-sm">{booking.location.pincode}</span>
                                  <span className="text-xs text-gray-500 capitalize">
                                    {booking.locationType?.replace('-', ' ') || 'N/A'}
                                  </span>
                                </div>
                              </div>
                            </td>
                ...
                {pagination && (
                  <div className="flex items-center justify-between mt-6">
                    <div className="text-sm text-gray-600">
                      ...
                      {Math.min(pagination.currentPage * 10, pagination.totalBookings)} of{' '}
